...
public class Game extends JPanel implements KeyListener, ActionListener {
	...
	public Game() {		
		mapGenerator = new MapGenerator(4, totalBricks / 4);
		addKeyListener(this);
		setFocusable(true);
	...
	public void paint(Graphics graphics) {    		
		// background
		Color backgroundColor = new Color(69, 178, 157);

		graphics.setColor(backgroundColor);
		graphics.fillRect(1, 1, 698, 592);
		
		// drawing map
		mapGenerator.draw((Graphics2D) graphics, backgroundColor);
			...
			A: for(int i = 0; i<mapGenerator.map.length; i++) {
				for(int j =0; j<mapGenerator.map[0].length; j++) {				
					if(mapGenerator.map[i][j] > 0) {
						//scores++;
						int brickX = j * mapGenerator.brickWidth + 25;
